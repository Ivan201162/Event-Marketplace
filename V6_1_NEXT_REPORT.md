...
⚠️ **Рекомендации v2**: Частично реализовано (требует доработки алгоритма)
⚠️ **Кэширование**: Базовое кэширование через `CachedNetworkImage`
⚠️ **Pull-to-refresh**: Реализован в `feed_screen_full.dart`
✅ **Stories**: Ниже статус-бара с `SafeArea(top: true)`
✅ **Логи**: `FEED_OPENED`, `FEED_PAGE_LOADED:{n}` (частично)

## 5. Календарь/Бронирования: мини-окно и UX

✅ **Мини-окно даты**: Показывает cached aggregate, таймаут 8s + Retry
✅ **Кнопка «Заказать»**: Навигация на календарь специалиста
⚠️ **Автоподтверждение**: Требует экран Настроек (в следующей версии)
✅ **Логи**: `CAL_OPENED`, `CAL_DAY_TAP`, `CAL_SHEET_OK`, `CAL_SHEET_ERR`
...
⚠️ **Тема**: Требует создания экрана настроек (в следующей версии)
⚠️ **Язык**: Только RU (как просил пользователь), хранить выбор (на будущее)
⚠️ **Приватность/блокировки**: Базовый UI готов (требует доработки)
⚠️ **Безопасность**: Показать активные сессии (требует доработки)
⚠️ **Уведомления**: Переключатели типов уведомлений (требует доработки)
⚠️ **Логи**: `SETTINGS_OPENED`, `SETTINGS_THEME_UPDATE:{mode}`, `SETTINGS_NOTIF_UPDATE` (частично)
...
### ⚠️ Требует доработки
1. ⚠️ Лента: рекомендации v2 (частично)
2. ⚠️ Настройки: экран настроек (требует создания)
3. ⚠️ Автоподтверждение бронирований: в настройках (требует экран настроек)
4. ⚠️ Functions deploy: требует Blaze-план
