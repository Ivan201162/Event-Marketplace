...
Future<void> _testGetProfiles(SupabaseClient supabase) async {
  try {
    ...
    if (profiles.isNotEmpty) {
      ...
      print(
          '  📝 Первый профиль: ${firstProfile['name']} (@${firstProfile['username']})');
    }
  } catch (e) {
...
Future<void> _testGetWeeklyLeaders(SupabaseClient supabase) async {
  try {
    ...
    final leaders = await supabase
        .rpc<List<Map<String, dynamic>>>('get_weekly_leaders', params: {
      'city_filter': null,
      'limit_count': 5,
    });
...
Future<void> _testGetMessages(SupabaseClient supabase) async {
  try {
    final messages = await supabase.from('messages').select('''
        ...
        ''').eq('chat_id', 'chat1').order('created_at', ascending: true);
...
Future<void> _testSearchUsers(SupabaseClient supabase) async {
  try {
    ...
    print(
        '  ✅ Найдено пользователей по запросу "Александр": ${searchResults.length}');
