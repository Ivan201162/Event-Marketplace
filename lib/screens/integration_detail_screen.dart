...
class _IntegrationDetailScreenState extends ConsumerState<IntegrationDetailScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [IconButton(icon: const Icon(Icons.share), onPressed: _shareIntegration)],
    ),
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Icon(widget.integration.typeIcon, size: 20, color: widget.integration.typeColor),
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(widget.integration.description, style: const TextStyle(fontSize: 16, height: 1.5)),
        ],
      ),
    ),
  );
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Разрешения', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
              ...
              (permission) => Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Expanded(child: Text(permission, style: const TextStyle(fontSize: 14))),
                  ],
                ),
              ),
            ),
            ...
            ...widget.integration.config.entries.map(
              (entry) => ListTile(
                ...
                onTap: () {
                  ...
                  ScaffoldMessenger.of(
                    context,
                  ).showSnackBar(SnackBar(content: Text('Редактирование ${entry.key}')));
                },
              ),
            ),
          ],
  ...
  Future<void> _syncIntegration() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Синхронизация завершена'), backgroundColor: Colors.green),
      );
    } on Exception catch (e) {
      ScaffoldMessenger.of(
        context,
  ...
  Future<void> _openWebsite() async {
    if (widget.integration.websiteUrl != null) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Не удалось открыть сайт: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  void _shareIntegration() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Интеграция скопирована в буфер обмена')));
  }
}
