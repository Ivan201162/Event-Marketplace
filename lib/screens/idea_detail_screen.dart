...
class _IdeaDetailScreenState extends ConsumerState<IdeaDetailScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.share), onPressed: _shareIdea)],
      ),
      body: Column(
        children: [
          // Медиа контент
                      ...
                      child: GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => VideoReelsViewer(initialIdea: widget.idea),
                            ),
                          );
                        },
                        child: Container(
    ...
    child: Column(
      ...
      children: [
          ...
          children: [
            CircleAvatar(
              ...
              backgroundImage: widget.idea.authorAvatar != null
                  ...
                  : null,
              child: widget.idea.authorAvatar == null ? const Icon(Icons.person, size: 16) : null,
            ),
        ...
        if (widget.idea.description.isNotEmpty) ...[
          Text(widget.idea.description, style: const TextStyle(fontSize: 16)),
          const SizedBox(height: 12),
        ],
                  ...
                  GestureDetector(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(comment.likes.toString(), style: const TextStyle(fontSize: 12)),
                      ],
                    ),
                  ),
                ],
  ...
  Future<void> _shareIdea() async {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => ShareIdeaScreen(idea: widget.idea)),
    );
  }
  ...
  Future<void> _addComment(user) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Ошибка добавления комментария: $e')));
    } finally {
      setState(() {
        _isCommenting = false;
      });
