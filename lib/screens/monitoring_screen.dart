...
class _MonitoringScreenState extends ConsumerState<MonitoringScreen> {
  ...
  Widget _buildMetricsTab() => Column(
    children: [
      ...
      ResponsiveCard(
        child: Row(
          children: [
            Text('Метрики системы', style: Theme.of(context).textTheme.titleMedium),
            const Spacer(),
            DropdownButton<String?>(
              hint: const Text('Все категории'),
              items: const [
                DropdownMenuItem<String?>(child: Text('Все категории')),
                DropdownMenuItem<String?>(value: 'system', child: Text('Система')),
                DropdownMenuItem<String?>(value: 'network', child: Text('Сеть')),
                DropdownMenuItem<String?>(value: 'database', child: Text('База данных')),
                DropdownMenuItem<String?>(value: 'users', child: Text('Пользователи')),
                DropdownMenuItem<String?>(value: 'errors', child: Text('Ошибки')),
              ],
              onChanged: (value) {
  ...
  Widget _buildAlertsTab() => Column(
    children: [
      ...
      ResponsiveCard(
        child: Row(
          children: [
            Text('Алерты мониторинга', style: Theme.of(context).textTheme.titleMedium),
            const Spacer(),
            ElevatedButton.icon(
    ...
    return ResponsiveCard(
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Text(alert.severity.icon, style: const TextStyle(fontSize: 24)),
              const SizedBox(width: 8),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  const PopupMenuItem(
                    value: 'disable',
                    child: ListTile(leading: Icon(Icons.block), title: Text('Отключить')),
                  ),
                ],
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              _buildInfoChip('Каналы', '${alert.notificationChannels.length}', Colors.green),
            ],
          ),
  ...
  Widget _buildDashboardsTab() => Column(
    children: [
      ...
      ResponsiveCard(
        child: Row(
          children: [
            Text('Дашборды мониторинга', style: Theme.of(context).textTheme.titleMedium),
            const Spacer(),
            ElevatedButton.icon(
  ...
  Widget _buildDashboardCard(MonitoringDashboard dashboard) => ResponsiveCard(
    child: Column(
      ...
      children: [
        ...
        Row(
          children: [
            _buildInfoChip('Метрики', '${dashboard.metricCount}', Colors.blue),
            const SizedBox(width: 8),
            _buildInfoChip('Алерты', '${dashboard.alertCount}', Colors.orange),
          ],
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              if (alert.notificationChannels.isNotEmpty) ...[
                const SizedBox(height: 8),
                Text('Каналы уведомлений: ${alert.notificationChannels.join(', ')}'),
              ],
            ],
          ),
        ),
  ...
  void _editAlert(MonitoringAlert alert) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Редактирование алерта "${alert.name}" будет реализовано')),
    );
  }
  ...
  void _resolveAlert(MonitoringAlert alert) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Решение алерта "${alert.name}" будет реализовано')));
  }
  ...
  void _disableAlert(MonitoringAlert alert) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Отключение алерта "${alert.name}" будет реализовано')));
  }
  ...
  void _exportDashboard(MonitoringDashboard dashboard) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Экспорт дашборда "${dashboard.name}" будет реализован')),
    );
  }
  ...
  void _showCreateAlertDialog() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Создание алерта будет реализовано')));
  }
