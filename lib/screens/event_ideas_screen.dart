...
class _EventIdeasScreenState extends ConsumerState<EventIdeasScreen>
  ...
  Future<void> _loadFavoriteIdeas() async {
    try {
      final favorites = await _ideasService.getFavoriteIdeas('current_user_id'); // TODO: Получить реальный ID
      setState(() {
  ...
  Widget _buildFavoriteIdeasTab() {
    return _favoriteIdeas.isEmpty
  ...
  Widget _buildRecommendedIdeasTab() {
    return _recommendedIdeas.isEmpty
  ...
  Future<void> _toggleLike(EventIdea idea) async {
    try {
      final userId = 'current_user_id'; // TODO: Получить реальный ID
      ...
      _updateIdeaInLists(idea.id, (idea) => idea.copyWith(
...
class _IdeaDetailScreenState extends State<IdeaDetailScreen> {
  ...
  Widget _buildComment(IdeaComment comment) {
    return Card(
