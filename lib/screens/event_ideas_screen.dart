...
class _EventIdeasScreenState extends ConsumerState<EventIdeasScreen>
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [IconButton(onPressed: _showAddIdeaDialog, icon: const Icon(Icons.add))],
    ),
    body: TabBarView(
      controller: _tabController,
      children: [_buildAllIdeasTab(), _buildFavoriteIdeasTab(), _buildRecommendedIdeasTab()],
    ),
  );
  ...
  Widget _buildAllIdeasTab() => Column(
    children: [
      ...
      Container(
        ...
        child: Column(
          children: [
            IdeaSearchBar(onSearch: _onSearch, initialValue: _searchQuery),
  ...
  void _showAddIdeaDialog() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Добавление идеи будет реализовано')));
  }
  ...
  Widget _buildIdeaDetails() => Card(
    child: Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Детали', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 12),
          if (widget.idea.budget != null)
            _buildDetailRow('Бюджет', '${widget.idea.budget!.toStringAsFixed(0)} ₽'),
          if (widget.idea.duration != null)
            _buildDetailRow('Длительность', '${widget.idea.duration} часов'),
          if (widget.idea.guestCount != null)
            _buildDetailRow('Количество гостей', '${widget.idea.guestCount} человек'),
