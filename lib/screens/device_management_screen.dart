...
class _DeviceManagementScreenState extends ConsumerState<DeviceManagementScreen> {
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Это устройство', style: TextStyle(fontSize: 14, color: Colors.grey)),
                  ],
                ),
              ),
              Container(
  ...
  Widget _buildDevicesList(List<SecurityDevice> devices) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('Все устройства', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
      const SizedBox(height: 12),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildDeviceInfoItem(icon: Icons.info, label: 'ОС', value: device.osVersion),
              ),
              Expanded(
                child: _buildDeviceInfoItem(
                  icon: Icons.apps,
  ...
  Future<void> _unblockDevice(SecurityDevice device) async {
    try {
      ...
      if (currentUser != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Устройство разблокировано'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(
