...
class _OrganizersListScreenState extends ConsumerState<OrganizersListScreen> {
    ...
    setState(() {
      _filteredOrganizers = _organizers.where((organizer) {
        ...
        final searchMatch =
            ...
            organizer.categories.any((category) => category.toLowerCase().contains(searchQuery));
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [IconButton(icon: const Icon(Icons.refresh), onPressed: _loadOrganizers)],
    ),
    body: Column(
      children: [
        // Поиск и фильтры
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Организаторы не найдены', style: TextStyle(fontSize: 18, color: Colors.grey)),
            Text('Попробуйте изменить параметры поиска', style: TextStyle(color: Colors.grey)),
          ],
        ),
      );
    }
              ...
              children: [
                ...
                CircleAvatar(
                  ...
                  backgroundImage: organizer.logoUrl != null
                      ...
                      : null,
                  child: organizer.logoUrl == null ? const Icon(Icons.business, size: 30) : null,
                ),
                const SizedBox(width: 16),
