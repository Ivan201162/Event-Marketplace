...
class _ReviewFormScreenState extends ConsumerState<ReviewFormScreen> {
  ...
  Widget _buildTargetInfo() => Container(
    ...
    child: Row(
      children: [
        Icon(_getTargetIcon(), color: Theme.of(context).colorScheme.primary, size: 32),
  ...
  Future<void> _addImage() async {
    if (!FeatureFlags.fileUploadEnabled) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Загрузка изображений отключена')));
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ошибка загрузки изображения: $e'), backgroundColor: Colors.red),
      );
    } finally {
