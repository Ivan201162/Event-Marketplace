...
class _PaymentScreenState extends ConsumerState<PaymentScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: TabBarView(
      controller: _tabController,
      children: [_buildPendingPaymentsTab(), _buildCompletedPaymentsTab(), _buildStatisticsTab()],
    ),
  );
        ...
        return const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('Нет ожидающих платежей', style: TextStyle(fontSize: 18, color: Colors.grey)),
            ],
          ),
        );
      }
        ...
        return const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('Нет завершенных платежей', style: TextStyle(fontSize: 18, color: Colors.grey)),
            ],
          ),
        );
      }
  ...
  Future<void> _processPaymentWithProvider(Payment payment, PaymentProvider provider) async {
    try {
      final result = await _paymentService.processPaymentWithProvider(
        ...
        metadata: {'bookingId': payment.bookingId, 'customerId': payment.customerId},
      );
      ...
      if (result['success'] == true) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Перенаправление на страницу оплаты...')));
      } else {
        ScaffoldMessenger.of(
          context,
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Ошибка обработки платежа: $e')));
    }
  }
  ...
  Future<void> _cancelPayment(Payment payment) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Платеж отменен')));
      setState(() {});
    } on Exception catch (e) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Ошибка отмены платежа: $e')));
    }
  }
}
