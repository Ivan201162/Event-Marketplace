...
class OrganizerProfileScreen extends ConsumerStatefulWidget {
  const OrganizerProfileScreen({super.key, this.organizerId, this.isEditMode = false});
...
class _OrganizerProfileScreenState extends ConsumerState<OrganizerProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditing ? 'Редактирование профиля' : 'Профиль организатора'),
        actions: [
          ...
          if (_isEditing) ...[
            ...
            TextButton(onPressed: _saveProfile, child: const Text('Сохранить')),
          ],
        ],
      ),
    ...
    child: Padding(
      ...
      child: Row(
        children: [
          CircleAvatar(
            radius: 40,
            backgroundImage: _profile!.logoUrl != null ? NetworkImage(_profile!.logoUrl!) : null,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text('Специализации', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          if (_profile!.specializations.isEmpty)
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: _profile!.specializations.map((spec) => Chip(label: Text(spec))).toList(),
            ),
        ],
      ),
    ),
        ...
        children: [
          ...
          if (!_profile!.hasPortfolio)
            ...
            if (_profile!.portfolioImages.isNotEmpty) ...[
              const Text('Изображения:', style: TextStyle(fontWeight: FontWeight.w500)),
              const SizedBox(height: 8),
              SizedBox(
                height: 100,
            ...
            if (_profile!.pastEvents.isNotEmpty) ...[
              const Text('Завершенные проекты:', style: TextStyle(fontWeight: FontWeight.w500)),
              const SizedBox(height: 8),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _buildStatItem('Рейтинг', _profile!.formattedRating, Icons.star),
              _buildStatItem('Отзывы', _profile!.reviewCount.toString(), Icons.rate_review),
              _buildStatItem('Проекты', _profile!.projectCount.toString(), Icons.work),
              _buildStatItem('Команда', _profile!.teamMembers.length.toString(), Icons.group),
            ],
          ),
        ],
      ),
      ...
      child: Column(
        ...
        children: [
          ...
          _buildEditSection('Основная информация', [
            ...
            _buildTextField(_descriptionController, 'Описание', false, maxLines: 3),
          ...
          // Контактная информация
          _buildEditSection('Контактная информация', [
          ...
          _buildEditSection('Условия работы', [
            Row(
              children: [
                Expanded(
            ...
            _buildTextField(_responseTimeController, 'Время ответа', false),
          ]),
          const SizedBox(height: 24),
