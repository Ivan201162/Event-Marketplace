...
class _IdeaCollectionsScreenState extends ConsumerState<IdeaCollectionsScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [IconButton(icon: const Icon(Icons.add), onPressed: _createCollection)],
    ),
  ...
  Widget _buildEmptyState() => Center(
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        const Text('Нет коллекций', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  void _editCollection(IdeaCollection collection) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Редактирование: ${collection.name}')));
  }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(SnackBar(content: Text('Коллекция "${collection.name}" удалена')));
            },
...
class _CreateIdeaCollectionScreenState extends ConsumerState<CreateIdeaCollectionScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [
        TextButton(onPressed: _isLoading ? null : _saveCollection, child: const Text('Сохранить')),
      ],
    ),
    body: Form(
      key: _formKey,
    ...
    try {
      ...
      if (collectionId != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Коллекция успешно создана'), backgroundColor: Colors.green),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Ошибка создания коллекции'), backgroundColor: Colors.red),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(
