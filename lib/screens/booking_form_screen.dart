...
class _BookingFormScreenState extends ConsumerState<BookingFormScreen> {
    ...
    child: Form(
      ...
      child: Column(
        ...
        children: [
                ...
                children: [
                  CircleAvatar(
                    ...
                    backgroundImage: specialist.avatar != null
                        ...
                        : null,
                    child: specialist.avatar == null ? const Icon(Icons.person, size: 30) : null,
                  ),
          ...
          Text('Тип бронирования', style: Theme.of(context).textTheme.titleMedium),
          ...
          // Выбор даты
          Text('Выберите дату', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          InkWell(
            onTap: _selectDate,
          ...
          // Выбор времени
          Text('Выберите время', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          InkWell(
            onTap: _selectTime,
            child: InputDecorator(
          ...
          // Продолжительность
          Text('Продолжительность (часы)', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          Slider(
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Аванс (%)', style: Theme.of(context).textTheme.titleSmall),
                  const SizedBox(height: 8),
                  Slider(
          ...
          Text('Дополнительные пожелания', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          TextFormField(
  ...
  Widget _buildTeamManagement() => Card(
    child: Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Управление командой', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 16),
          if (_selectedTeamId == null) ...[
            const Text('Создайте команду специалистов для этого мероприятия'),
            const SizedBox(height: 16),
            SizedBox(
              width: double.infinity,
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('Команда создана успешно')));
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('Ошибка создания команды: $e')));
        }
      }
    }
  }
  ...
  void _editTeam(SpecialistTeam team) {
    Navigator.of(
      context,
    ).push(MaterialPageRoute<void>(builder: (context) => TeamScreen(teamId: team.id)));
  }
  ...
  void _submitBooking() {
    ...
    if (_isTeamBooking && _selectedTeamId == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Создайте команду специалистов')));
      return;
    }
