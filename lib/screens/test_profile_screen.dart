...
class _TestProfileScreenState extends State<TestProfileScreen> {
  ...
  Future<void> _clearTestData() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Тестовые данные очищены'), backgroundColor: Colors.orange),
        );
      }
    } catch (e) {
      if (mounted) {
    ...
    if (_testSpecialistId != null) {
      Navigator.of(context).push(
        MaterialPageRoute<void>(
          builder: (context) => SpecialistProfileInstagramScreen(specialistId: _testSpecialistId!),
        ),
      );
    }
  }
          ...
          Card(
            ...
            child: const Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Инструкция:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  SizedBox(height: 8),
