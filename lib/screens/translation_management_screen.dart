...
class _TranslationManagementScreenState extends ConsumerState<TranslationManagementScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [IconButton(icon: const Icon(Icons.add), onPressed: _addTranslation)],
    ),
    body: Column(
      children: [
        // Фильтры
  ...
  Widget _buildEmptyState() => const Center(
    child: Column(
      ...
      children: [
        ...
        SizedBox(height: 16),
        Text('Нет переводов', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        SizedBox(height: 8),
        Text(
  ...
  Map<String, String> _filterTranslations(Map<String, String> translations) {
    ...
    if (_selectedCategory != 'all') {
      filtered = Map.fromEntries(
        filtered.entries.where((entry) => _getCategoryFromKey(entry.key) == _selectedCategory),
      );
    }
      ...
      builder: (context) => _TranslationDialog(
        ...
        onSave: (key, value) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Функция добавления перевода будет доступна в следующих обновлениях'),
            ),
          );
        },
      ),
  ...
  void _copyTranslation(String key, String value) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Функция копирования перевода будет доступна в следующих обновлениях'),
      ),
    );
  }
}
...
class _TranslationDialogState extends State<_TranslationDialog> {
  ...
  Widget build(BuildContext context) => AlertDialog(
    ...
    actions: [
      ...
      ElevatedButton(onPressed: _saveTranslation, child: const Text('Сохранить')),
    ],
  );
