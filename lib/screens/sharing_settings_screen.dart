...
class _SharingSettingsScreenState extends ConsumerState<SharingSettingsScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Настройки шаринга')),
  ...
  Widget _buildSharingTemplates() => Card(
    child: Padding(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildSharingTemplate({
    ...
    child: Column(
      ...
      children: [
        ...
        Text(template, style: TextStyle(fontSize: 14, color: Colors.grey[600])),
      ],
    ),
  );
...
''';
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Не удалось поделиться профилем: $e'), backgroundColor: Colors.red),
      );
    }
  }
...
''';
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Не удалось поделиться событием: $e'), backgroundColor: Colors.red),
      );
    }
  }
...
''';
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Не удалось поделиться отзывом: $e'), backgroundColor: Colors.red),
      );
    }
  }
