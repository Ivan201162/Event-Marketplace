...
class _SpecialistProfileScreenState
    ...
    return Consumer(
      builder: (context, ref, child) {
        ...
        return postsAsync.when(
          data: (posts) {
            if (posts.isEmpty) {
              return _buildEmptyState('Нет постов',
  ...
  Widget _buildAlbumsTab() {
    return ResponsiveList(
      children: [
        _buildAlbumCard(
            'Свадебные фото', '24 фото', 'https://via.placeholder.com/300x200'),
        _buildAlbumCard(
            'Портреты', '18 фото', 'https://via.placeholder.com/300x200'),
        _buildAlbumCard(
            'Семейные фото', '32 фото', 'https://via.placeholder.com/300x200'),
        _buildAlbumCard(
            'Корпоративы', '15 фото', 'https://via.placeholder.com/300x200'),
      ],
    );
  }
          ...
          children: [
            ...
            Row(
              children: [
                CircleAvatar(
                  radius: 20,
                  backgroundImage: NetworkImage(post.specialistPhotoUrl ??
    ...
    return ResponsiveCard(
      child: Column(
        ...
        children: [
          ...
          ResponsiveGrid(
            crossAxisCount: 3,
            children: List.generate(9, (index) => _buildPortfolioItem(index)),
          ),
        ],
      ),
    );
    ...
    return ResponsiveCard(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _buildReviewItem('Мария Иванова',
              'Отличная работа! Очень довольна результатом.', '5.0'),
          _buildReviewItem('Алексей Петров',
              'Профессиональный подход, качественные фото.', '5.0'),
          _buildReviewItem('Елена Сидорова',
              'Рекомендую всем! Очень внимательный к деталям.', '5.0'),
        ],
      ),
    );
  }
  ...
  Widget _buildTopPostItem(String title, String likes) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 8),
      child: Row(
        children: [
  ...
  Widget _buildReviewItem(String name, String review, String rating) {
    return Padding(
