...
class _BackupManagementScreenState extends ConsumerState<BackupManagementScreen> {
  ...
  Widget _buildTabs() => ResponsiveCard(
    child: Row(
      children: [
        ...
        Expanded(child: _buildTabButton('restores', 'Восстановления', Icons.restore)),
  ...
  Widget _buildBackupsTab() => Column(
    children: [
      // Заголовок с кнопками
      ResponsiveCard(
        child: Row(
          children: [
            Text('Бэкапы', style: Theme.of(context).textTheme.titleMedium),
            const Spacer(),
            ElevatedButton.icon(
  ...
  Widget _buildBackupCard(Backup backup) => ResponsiveCard(
    child: Column(
      ...
      children: [
        ...
        Row(
          children: [
            Icon(_getBackupIcon(backup.type), color: _getBackupColor(backup.type), size: 24),
            const SizedBox(width: 8),
            Expanded(child: Text(backup.name, style: Theme.of(context).textTheme.titleMedium)),
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                if (backup.isCompleted) ...[
                  ...
                  const PopupMenuItem(
                    value: 'restore',
                    child: ListTile(leading: Icon(Icons.restore), title: Text('Восстановить')),
                  ),
                ],
                const PopupMenuItem(
                  value: 'delete',
        ...
        Row(
          children: [
            ...
            if (backup.fileSize != null)
              _buildInfoChip('Размер', backup.formattedFileSize, Colors.green),
          ],
        ),
  ...
  Widget _buildRestoresTab() => Column(
    children: [
      // Заголовок
      ResponsiveCard(
        child: Row(
          children: [
            Text('Восстановления', style: Theme.of(context).textTheme.titleMedium),
            const Spacer(),
            ElevatedButton.icon(
  ...
  Widget _buildRestoreCard(Restore restore) => ResponsiveCard(
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Заголовок
        Row(
          children: [
            Icon(_getRestoreIcon(restore.type), color: _getRestoreColor(restore.type), size: 24),
            const SizedBox(width: 8),
            Expanded(child: Text(restore.name, style: Theme.of(context).textTheme.titleMedium)),
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 8),
            _buildInfoChip('Бэкап ID', restore.backupId.substring(0, 8), Colors.green),
          ],
        ),
  ...
  Widget _buildStatisticsTab() => FutureBuilder<BackupStatistics>(
          ...
          children: [
            ...
            ResponsiveCard(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Статистика бэкапов', style: Theme.of(context).textTheme.titleMedium),
            ...
            ResponsiveCard(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Бэкапы по типам', style: Theme.of(context).textTheme.titleMedium),
                  const SizedBox(height: 16),
  ...
  Future<void> _loadData() async {
    ...
    try {
      await Future.wait([_loadBackups(), _loadRestores()]);
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _loadBackups() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ошибка загрузки бэкапов: $e'), backgroundColor: Colors.red),
      );
    }
  }
  ...
  Future<void> _loadRestores() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ошибка загрузки восстановлений: $e'), backgroundColor: Colors.red),
      );
    }
  }
  ...
  void _createBackup() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Функция создания бэкапа будет реализована')));
  }
  ...
  void _downloadBackup(Backup backup) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Скачивание бэкапа "${backup.name}" будет реализовано')));
  }

  void _restoreBackup(Backup backup) {
    // TODO(developer): Реализовать восстановление из бэкапа
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Восстановление из бэкапа "${backup.name}" будет реализовано')),
    );
  }
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                _loadBackups();
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Бэкап удален'), backgroundColor: Colors.green),
                );
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
