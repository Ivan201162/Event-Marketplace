...
class _LocationSettingsScreenState extends ConsumerState<LocationSettingsScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Настройки геолокации')),
  ...
  Widget _buildLocationInfo(LocationData location) => Container(
    ...
    child: Column(
      ...
      children: [
        ...
        if (location.accuracy != null) _buildInfoRow('Точность', '${location.accuracy!.toInt()} м'),
        if (location.altitude != null) _buildInfoRow('Высота', '${location.altitude!.toInt()} м'),
        ...
        children: [
          Row(
            ...
            children: [
              ...
              TextButton(onPressed: _clearLocationHistory, child: const Text('Очистить')),
            ],
          ),
          const SizedBox(height: 16),
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      if (location != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Местоположение обновлено'), backgroundColor: Colors.green),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Адрес: $address'),
              const SizedBox(height: 8),
              const Text('Полный адрес:', style: TextStyle(fontWeight: FontWeight.bold)),
              Text(address),
            ],
          ),
          actions: [
