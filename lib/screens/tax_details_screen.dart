...
class _TaxDetailsScreenState extends ConsumerState<TaxDetailsScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: TabBarView(
      controller: _tabController,
      children: [_buildTaxRecordsTab(), _buildStatisticsTab()],
    ),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text('Добавьте первую запись о налоге', style: TextStyle(color: Colors.grey)),
            const SizedBox(height: 16),
            ElevatedButton.icon(
              onPressed: _showAddTaxDialog,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text(taxInfo.taxTypeIcon, style: const TextStyle(fontSize: 24)),
  ...
  Widget _buildTaxInfoItem(String label, String value, IconData icon, Color color) => Column(
    children: [
  ...
  Widget _buildStatisticsTab() => FutureBuilder<Map<String, dynamic>>(
    ...
    builder: (context, snapshot) {
      ...
      return SingleChildScrollView(
        ...
        child: Column(
          children: [
            _buildStatCard('Общая статистика', [
              _buildStatItem(
                'Общий доход',
            ...
            const SizedBox(height: 16),
            _buildStatCard('Прогресс оплаты', [
  ...
  Widget _buildStatItem(String label, String value, IconData icon, Color color) => Padding(
  ...
  void _markAsPaid(TaxInfo taxInfo) {
    showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Вы уверены, что хотите отметить этот налог как оплаченный?'),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Отмена')),
          ElevatedButton(
            onPressed: () async {
