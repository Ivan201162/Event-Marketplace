...
class _MediaStorageTestScreenState extends State<MediaStorageTestScreen> {
  ...
  Widget _buildTestInfo() => Container(
    ...
    child: Column(
      ...
      children: [
        ...
        Text('Booking ID: $_testBookingId', style: const TextStyle(fontSize: 14)),
        Text('Specialist ID: $_testSpecialistId', style: const TextStyle(fontSize: 14)),
        const SizedBox(height: 8),
        const Text(
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            ...
            icon: Icon(_showUploadWidget ? Icons.visibility_off : Icons.visibility),
            label: Text(_showUploadWidget ? 'Скрыть загрузку' : 'Показать загрузку'),
        ...
        Expanded(
          child: ElevatedButton.icon(
            ...
            icon: Icon(_showGalleryWidget ? Icons.visibility_off : Icons.visibility),
            label: Text(_showGalleryWidget ? 'Скрыть галерею' : 'Показать галерею'),
            style: ElevatedButton.styleFrom(
      ...
      MediaUploadWidget(
        ...
        onUploadComplete: () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Загрузка завершена!'), backgroundColor: Colors.green),
          );
          // Обновляем галерею
          setState(() {});
        },
        onUploadError: (error) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Ошибка загрузки: $error'), backgroundColor: Colors.red),
          );
        },
      ),
    ],
  ...
  Widget _buildGallerySection() => Column(
    ...
    children: [
      ...
      const SizedBox(height: 8),
      MediaGalleryWidget(bookingId: _testBookingId, specialistId: _testSpecialistId),
    ],
  );
}
