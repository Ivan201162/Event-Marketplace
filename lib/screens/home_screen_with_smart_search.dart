...
class _HomeScreenWithSmartSearchState extends ConsumerState<HomeScreenWithSmartSearch> {
  ...
  void _openSmartSearch() {
    Navigator.push(context, MaterialPageRoute(builder: (context) => const SmartSearchScreen()));
  }
  ...
  Future<void> _openAIAssistant() async {
    ...
    if (mounted) {
      showDialog(
        context: context,
        builder: (context) => AIAssistantDialog(conversation: conversation, onClose: () {}),
      );
    }
  }
  ...
  Widget _buildSpecialistsList(
    ...
    if (specialists.isEmpty) {
      return const Center(
        child: Text('Специалисты не найдены', style: TextStyle(color: Colors.grey)),
      );
    }
