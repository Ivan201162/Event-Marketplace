...
class _CreateReviewExtendedScreenState extends ConsumerState<CreateReviewExtendedScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [
        TextButton(onPressed: _isLoading ? null : _submitReview, child: const Text('Отправить')),
      ],
    ),
    body: Form(
      key: _formKey,
  ...
  Widget _buildOverallRating() => Card(
    child: Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Общая оценка', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  Widget _buildRatingSlider(String label, double value, Function(double) onChanged) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Row(
      ...
      Slider(value: value, min: 1, max: 5, divisions: 40, onChanged: onChanged),
    ],
  );
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Text('Медиа', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
              Text('${_media.length}/10', style: TextStyle(color: Colors.grey[600])),
            ],
          ),
          const SizedBox(height: 16),
  ...
  Widget _buildTagsSection() => Card(
    child: Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Теги', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          ...
          Wrap(
            ...
            children: [
              ...
              'Рекомендую',
              'Отличная работа',
            ].map((tag) => ActionChip(label: Text(tag), onPressed: () => _addTag(tag))).toList(),
          ),
        ],
      ),
    ),
