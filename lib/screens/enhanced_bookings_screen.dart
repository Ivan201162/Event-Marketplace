...
class _EnhancedBookingsScreenState extends ConsumerState<EnhancedBookingsScreen>
                    ...
                    child: ListView(
                      children: [
                        ...
                        _buildDesktopTabItem(
                            1, 'На рассмотрении', Icons.schedule),
                        _buildDesktopTabItem(
                            2, 'Подтверждено', Icons.check_circle_outline),
                        _buildDesktopTabItem(
                            3, 'Завершено', Icons.check_circle),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildAllBookingsTab() {
    return _buildBookingsList([
      ...
      _createMockBooking(
          '2', 'Видеограф на корпоратив', BookingStatus.confirmed),
      _createMockBooking(
          '3', 'Диджей на день рождения', BookingStatus.completed),
  ...
  void _editBooking(Booking booking) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content:
              Text('Редактирование бронирования: ${booking.specialistName}')),
    );
  }
  ...
  Booking _createMockBooking(
      String id, String specialistName, BookingStatus status) {
    return Booking(
      id: id,
