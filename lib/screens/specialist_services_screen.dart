...
class _SpecialistServicesScreenState extends ConsumerState<SpecialistServicesScreen> {
  ...
  Widget _buildMobileLayout(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [
        IconButton(icon: const Icon(Icons.add), onPressed: () => _showAddServiceDialog(context)),
      ],
    ),
    body: _buildContent(),
  );
  ...
  Widget _buildTabletLayout(BuildContext context) => Scaffold(
    ...
    body: ResponsiveContainer(child: _buildContent()),
  );
          ...
          children: [
            const Row(
              children: [
                ...
                SizedBox(width: 12),
                Expanded(child: ResponsiveText('Управление услугами и ценами', isTitle: true)),
              ],
            ),
              ...
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    const Text('Добавьте услуги, чтобы клиенты могли их заказать'),
                    const SizedBox(height: 16),
                    ElevatedButton.icon(
                      onPressed: () => _showAddServiceDialog(context),
                      icon: const Icon(Icons.add),
    ...
    child: Column(
      ...
      children: [
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ResponsiveText(service.serviceName, isTitle: true),
                  const SizedBox(height: 4),
                  ResponsiveText(service.description, isSubtitle: true),
                ],
              ),
            ),
        ...
        if (service.pricingDetails.isNotEmpty) ...[
          const ResponsiveText('Детали ценообразования:', isSubtitle: true),
          const SizedBox(height: 4),
          ...service.pricingDetails.entries.map(
            (entry) => Padding(
              padding: const EdgeInsets.only(left: 16, bottom: 2),
              child: ResponsiveText('${entry.key}: ${entry.value}', isSubtitle: true),
            ),
          ),
        ],
      ],
  ...
  void _showAddServiceDialog(BuildContext context) {
    showDialog<void>(
      context: context,
      builder: (context) =>
          _ServiceDialog(specialistId: widget.specialistId, onServiceAdded: () => setState(() {})),
    );
  }
  ...
  void _showDeleteServiceDialog(BuildContext context, SpecialistService service) {
    showDialog<void>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Удалить услугу'),
        content: Text('Вы уверены, что хотите удалить услугу "${service.serviceName}"?'),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                await _serviceService.deleteSpecialistService(widget.specialistId, service.id);
                setState(() {});
...
class _ServiceDialogState extends State<_ServiceDialog> {
  ...
  Widget build(BuildContext context) => AlertDialog(
    title: Text(widget.service == null ? 'Добавить услугу' : 'Редактировать услугу'),
    content: Form(
      key: _formKey,
      child: Column(
  ...
  Future<void> _saveService() async {
    ...
    try {
      ...
      if (widget.service == null) {
        await _serviceService.createSpecialistService(widget.specialistId, service);
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Услуга обновлена')));
      }
