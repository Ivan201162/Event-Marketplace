...
class _SupportTicketDetailScreenState extends ConsumerState<SupportTicketDetailScreen> {
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              Text('Тикет #${widget.ticketId}', style: Theme.of(context).textTheme.titleLarge),
  ...
  void _closeTicket() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Тикет закрыт'), backgroundColor: Colors.green));
  }

  void _archiveTicket() {
    // TODO(developer): Архивировать тикет через SupportService
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Тикет архивирован'), backgroundColor: Colors.blue),
    );
  }
}
