...
class _SubscriptionsPageState extends ConsumerState<SubscriptionsPage>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [_buildPlansTab(currentUser), _buildMySubscriptionTab(currentUser)],
      ),
    );
  }
  ...
  Widget _buildMySubscriptionTab(currentUser) => StreamBuilder<Subscription?>(
    stream: _subscriptionService.getUserSubscription(currentUser.id, 'default_specialist'),
  ...
  Widget _buildPeriodSelector() => Container(
    ...
    child: Row(
      children: [
        Expanded(child: _buildPeriodButton('Месячная', SubscriptionPeriod.monthly)),
        Expanded(child: _buildPeriodButton('Годовая', SubscriptionPeriod.yearly)),
      ],
    ),
  );
  ...
  Widget _buildNoSubscriptionView() => Center(
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        Text('У вас нет активной подписки', style: Theme.of(context).textTheme.headlineSmall),
        const SizedBox(height: 8),
        const Text(
  ...
  Widget _buildCurrentPlanCard(Subscription subscription, SubscriptionPlan? plan) => Card(
    child: Padding(
      padding: const EdgeInsets.all(20),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow('Дата начала', _formatDate(subscription.createdAt)),
          _buildDetailRow(
            'Дата окончания',
  ...
  Future<void> _selectPlan(SubscriptionPlan plan) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ошибка активации подписки: $e'), backgroundColor: Colors.red),
      );
    } finally {
  ...
  void _toggleAutoRenew(Subscription subscription) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Функция будет реализована в следующей версии')));
  }
