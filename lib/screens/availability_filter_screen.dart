...
class _AvailabilityFilterScreenState extends ConsumerState<AvailabilityFilterScreen>
  ...
  Widget build(BuildContext context) {
    if (!FeatureFlags.availabilityFilterEnabled) {
      return Scaffold(
        appBar: AppBar(title: const Text('Фильтр по занятости')),
        body: const Center(
          child: Column(
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [_buildFiltersTab(), _buildResultsTab(), _buildCalendarTab()],
      ),
    );
  }
  ...
  Widget _buildDateRangeFilter() => Card(
    child: Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Период поиска', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  Widget _buildDateField(String label, DateTime? date, Function(DateTime?) onChanged) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      ...
      child: Column(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            subtitle: const Text('Показать только специалистов с высокой доступностью'),
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Специалисты не найдены', style: TextStyle(fontSize: 18, color: Colors.grey)),
            SizedBox(height: 8),
            Text('Попробуйте изменить параметры фильтра', style: TextStyle(color: Colors.grey)),
          ],
        ),
      );
    }
  ...
  Widget _buildCalendarTab() => const Center(
    child: Column(
      ...
      children: [
        ...
        SizedBox(height: 16),
        Text('Календарь занятости', style: TextStyle(fontSize: 18, color: Colors.grey)),
        SizedBox(height: 8),
        Text('Функция будет добавлена в следующей версии', style: TextStyle(color: Colors.grey)),
      ],
    ),
  );
