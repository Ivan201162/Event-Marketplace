...
class _AnalyticsScreenState extends ConsumerState<AnalyticsScreen>
  ...
  Widget _buildIncomeTab() {
    return FutureBuilder<SpecialistAnalytics?>(
  ...
  Widget _buildIncomeOverview(SpecialistIncomeStats stats) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _buildIncomeChart(SpecialistIncomeStats stats) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _buildBookingsChart(SpecialistIncomeStats stats) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: _buildBarChart(
                stats.bookingsByMonth.map((k, v) => MapEntry(k, v.toDouble())),
                Colors.blue,
              ),
            ),
            const SizedBox(height: 16),
            Row(
              ...
              children: [
                ...
                _buildStatItem('Завершено', stats.completedBookings.toString()),
                _buildStatItem('Отменено', stats.cancelledBookings.toString()),
                _buildStatItem(
                  'Успешность',
  ...
  Widget _buildRatingOverview(SpecialistReviewStats stats) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _buildRatingDistribution(SpecialistReviewStats stats) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildRatingBar(
                5, stats.fiveStarReviews, stats.totalReviews, Colors.green),
            _buildRatingBar(4, stats.fourStarReviews, stats.totalReviews,
                Colors.lightGreen),
            _buildRatingBar(
                3, stats.threeStarReviews, stats.totalReviews, Colors.yellow),
            _buildRatingBar(
                2, stats.twoStarReviews, stats.totalReviews, Colors.orange),
            _buildRatingBar(
                1, stats.oneStarReviews, stats.totalReviews, Colors.red),
          ],
        ),
      ),
    );
  ...
  Widget _buildReviewsChart(SpecialistReviewStats stats) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _buildCommonTags(SpecialistReviewStats stats) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            if (stats.commonTags.isEmpty)
              ...
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: stats.commonTags.map((tag) {
                  return Chip(
                    label: Text(tag),
  ...
  Widget _buildComparisonOverview(Map<String, dynamic> data) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Future<SpecialistAnalytics?> _getAnalytics() async {
    if (_currentSpecialistId == null) {
      // TODO: Получить ID текущего специалиста из провайдера
      _currentSpecialistId = 'demo_specialist_id';
    }
    return await _analyticsService
        .getSpecialistAnalytics(_currentSpecialistId!);
  }
  ...
  Future<Map<String, dynamic>> _getComparativeAnalytics() async {
    ...
    return await _analyticsService
        .getComparativeAnalytics(_currentSpecialistId!);
  }
}
