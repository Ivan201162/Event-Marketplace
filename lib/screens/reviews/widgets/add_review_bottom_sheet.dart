      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: Text('Оставить отзыв', style: Theme.of(context).textTheme.titleLarge),
              ),
    ...
    children: [
      ...
      Row(
        children: List.generate(
          5,
          (index) =>
              Icon(index < _rating ? Icons.star : Icons.star_border, color: Colors.amber, size: 24),
        ),
      ),
    ],
  );
  ...
  Future<void> _submitReview() async {
    ...
    try {
      ...
      for (final imageFile in _selectedImages) {
        ...
        photoUrls.add('https://picsum.photos/400?random=${DateTime.now().millisecondsSinceEpoch}');
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Отзыв успешно добавлен!')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Ошибка при добавлении отзыва: $e')));
      }
    } finally {
      if (mounted) {
        setState(() {
