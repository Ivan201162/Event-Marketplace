...
class _DJPlaylistScreenState extends ConsumerState<DJPlaylistScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [_buildPlaylistsTab(), _buildMediaFilesTab(), _buildVKImportTab()],
      ),
    );
  }
        ...
        return const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('У вас пока нет плейлистов', style: TextStyle(fontSize: 18, color: Colors.grey)),
              SizedBox(height: 8),
              Text(
            ...
            children: [
              Container(
                ...
                child: playlist.coverImagePath != null
                    ? ClipRRect(
                        borderRadius: BorderRadius.circular(8),
                        child: Image.network(playlist.coverImagePath!, fit: BoxFit.cover),
                      )
                    : Icon(Icons.music_note, color: Colors.purple[600], size: 30),
              ),
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  PopupMenuItem(
                    ...
                    child: Row(
                      children: [
                        Icon(playlist.isPublic ? Icons.visibility_off : Icons.visibility),
                        const SizedBox(width: 8),
                        Text(playlist.isPublic ? 'Сделать приватным' : 'Сделать публичным'),
                      ],
                    ),
                  ),
                  const PopupMenuItem(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(Icons.play_arrow, size: 14, color: Colors.blue[600]),
                    const SizedBox(width: 4),
                    Text(
  ...
  Widget _buildMediaFilesTab() => StreamBuilder<List<MediaFile>>(
    ...
    builder: (context, snapshot) {
      ...
      if (snapshot.hasError) {
        return Center(child: Text('Ошибка загрузки медиафайлов: ${snapshot.error}'));
      }
  ...
  Widget _buildMediaFileCard(MediaFile mediaFile) => Card(
    ...
    child: ListTile(
      leading: Container(
        ...
        child: Icon(_getMediaTypeIcon(mediaFile.type), color: Colors.white, size: 24),
      ),
      title: Text(mediaFile.originalName, style: const TextStyle(fontWeight: FontWeight.w500)),
      subtitle: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${mediaFile.formattedFileSize} • ${mediaFile.formattedDuration}'),
      ...
      child: Column(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            subtitle: const Text('Другие пользователи смогут видеть этот плейлист'),
            value: _isPublic,
            onChanged: (value) {
              setState(() {
                _isPublic = value;
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Файл успешно загружен!'), backgroundColor: Colors.green),
          );
        } catch (e) {
          Navigator.of(context).pop(); // Закрываем диалог загрузки
          rethrow;
  ...
  Future<void> _importVKPlaylist() async {
    ...
    if (playlistName.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Введите название плейлиста'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Ошибка импорта: $e'), backgroundColor: Colors.red));
    }
  }
  ...
  Future<void> _togglePlaylistVisibility(DJPlaylist playlist) async {
    try {
      await _playlistService.updatePlaylist(playlist.id, {'isPublic': !playlist.isPublic});
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Плейлист удален'), backgroundColor: Colors.green),
                );
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _downloadMediaFile(MediaFile mediaFile) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Функция скачивания будет добавлена')));
  }
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Файл удален'), backgroundColor: Colors.green),
                );
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Ошибка удаления: $e'), backgroundColor: Colors.red),
