...
class _DJPlaylistScreenState extends ConsumerState<DJPlaylistScreen>
  ...
  Widget _buildPlaylistsTab() {
    return StreamBuilder<List<DJPlaylist>>(
  ...
  Widget _buildPlaylistCard(DJPlaylist playlist) {
    return Card(
  ...
  Widget _buildMediaFileCard(MediaFile mediaFile) {
    return Card(
      ...
      child: ListTile(
        leading: Container(
          ...
          child: Icon(
            _getMediaTypeIcon(mediaFile.type),
            color: Colors.white,
            size: 24,
          ),
        ),
        ...
        child: Column(
          ...
          children: [
            ...
            SwitchListTile(
              ...
              subtitle:
                  const Text('Другие пользователи смогут видеть этот плейлист'),
              value: _isPublic,
              onChanged: (value) {
                setState(() {
                  _isPublic = value;
  ...
  Future<void> _togglePlaylistVisibility(DJPlaylist playlist) async {
    try {
      await _playlistService.updatePlaylist(playlist.id, {
