...
class _GuestRegistrationScreenState extends ConsumerState<GuestRegistrationScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Регистрация гостя')),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: Text(_event!.location, style: const TextStyle(fontSize: 14))),
            ],
          ),
    ...
    child: Padding(
      ...
      child: Form(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Данные гостя', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
    ...
    try {
      ...
      if (event != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Событие не найдено'), backgroundColor: Colors.red),
        );
        Navigator.pop(context);
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ошибка загрузки события: $e'), backgroundColor: Colors.red),
      );
      Navigator.pop(context);
    }
  }
