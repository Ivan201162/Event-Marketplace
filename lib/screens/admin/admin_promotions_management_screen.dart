...
class _AdminPromotionsManagementScreen extends State<AdminPromotionsManagementScreen> {
  ...
  Future<void> _toggleCampaignStatus(MarketingCampaign campaign, MarketingCampaignStatus newStatus) async {
    try {
      // Mock admin data for testing
      const adminId = 'admin_123';
      const adminEmail = 'admin@example.com';
      ...
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Статус кампании изменен на ${_getCampaignStatusName(newStatus)}')),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteCampaign(MarketingCampaign campaign) async {
    ...
    if (confirmed == true) {
      try {
        await FirebaseFirestore.instance.collection('marketing_campaigns').doc(campaign.id).delete();
        ScaffoldMessenger.of(context).showSnackBar(
