...
class EventIdeasService {
  ...
  Future<List<EventIdea>> getPublishedIdeas({
    ...
    try {
      ...
      if (searchQuery != null && searchQuery.isNotEmpty) {
        ...
        AppLogger.logI(
          'Получено идей: ${filteredIdeas.length}',
          'event_ideas_service',
        );
  ...
  Future<List<EventIdea>> getUserIdeas(String userId) async {
    try {
      ...
      final ideas = snapshot.docs.map((doc) => EventIdea.fromMap(doc.data())).toList();

      AppLogger.logI(
        'Получено идей пользователя $userId: ${ideas.length}',
        'event_ideas_service',
      );
      return ideas;
    } on Exception catch (e) {
      AppLogger.logE(
        'Ошибка получения идей пользователя',
        'event_ideas_service',
        e,
      );
      rethrow;
  ...
  Future<void> likeIdea(String ideaId, String userId) async {
    try {
      ...
      final likeRef = _firestore.collection('idea_likes').doc('${ideaId}_$userId');
  ...
  Future<bool> isIdeaLiked(String ideaId, String userId) async {
    try {
      final doc = await _firestore.collection('idea_likes').doc('${ideaId}_$userId').get();
  ...
  Future<void> addToFavorites(
    ...
    try {
      ...
      await _firestore.collection('favorite_ideas').doc(favoriteId).set(favorite.toMap());

      AppLogger.logI(
        'Добавлена в избранное идея: $ideaId',
        'event_ideas_service',
      );
    } on Exception catch (e) {
  ...
  Future<void> removeFromFavorites(String ideaId, String userId) async {
    try {
      ...
      AppLogger.logI(
        'Убрана из избранного идея: $ideaId',
        'event_ideas_service',
      );
    } on Exception catch (e) {
  ...
  Future<List<EventIdea>> getFavoriteIdeas(String userId) async {
    try {
      ...
      final favoriteIds = snapshot.docs.map((doc) => doc.data()['ideaId'] as String).toList();
      ...
      AppLogger.logI(
        'Получено избранных идей: ${ideas.length}',
        'event_ideas_service',
      );
      return ideas;
    } on Exception catch (e) {
      AppLogger.logE(
        'Ошибка получения избранных идей',
        'event_ideas_service',
        e,
      );
      rethrow;
  ...
  Future<List<IdeaComment>> getIdeaComments(String ideaId) async {
    try {
      ...
      final comments = snapshot.docs.map((doc) => IdeaComment.fromMap(doc.data())).toList();
  ...
  Future<String> addComment({
    ...
    try {
      ...
      batch.update(
        _firestore.collection('event_ideas').doc(ideaId),
      ...
      AppLogger.logI(
        'Добавлен комментарий к идее: $ideaId',
        'event_ideas_service',
      );
      return commentId;
    } on Exception catch (e) {
  ...
  Future<List<EventIdea>> getSimilarIdeas(
    String ideaId, {
    int limit = 5,
  }) async {
    try {
      ...
      final similarIdeas = snapshot.docs.map((doc) => EventIdea.fromMap(doc.data())).toList();

      AppLogger.logI(
        'Получено похожих идей: ${similarIdeas.length}',
        'event_ideas_service',
      );
      return similarIdeas;
    } on Exception catch (e) {
  ...
  Future<List<EventIdea>> getRecommendedIdeas(
    String userId, {
    int limit = 10,
  }) async {
    try {
      ...
      AppLogger.logE(
        'Ошибка получения рекомендуемых идей',
        'event_ideas_service',
        e,
      );
      return [];
