...
import 'package:event_marketplace_app/models/app_notification.dart' as model;
import 'package:event_marketplace_app/models/booking.dart';
import 'package:event_marketplace_app/models/calendar_event.dart' as calendar;
import 'package:event_marketplace_app/models/notification_template.dart' as template;
...
class FirestoreService {
  ...
  Future<void> addTestBookings() async {
    ...
    final b1 = Booking(
      id: 'b_test_1',
      specialistId: 's1',
      specialistName: 'Тестовый специалист 1',
      clientId: 'u_customer1',
      clientName: 'Тестовый клиент 1',
      service: 'Тестовая услуга 1',
      date: now,
      time: '12:00',
      duration: 2,
      totalPrice: 10000,
    ...
    final b2 = Booking(
      ...
      specialistId: 's1',
      specialistName: 'Тестовый специалист 1',
      clientId: 'u_customer2',
      clientName: 'Тестовый клиент 2',
      service: 'Тестовая услуга 2',
      date: now,
      time: '14:00',
      duration: 3,
      totalPrice: 15000,
      notes: 'Тестовое бронирование 2',
  ...
  Future<void> addOrUpdateBookingWithCalendar(Booking booking) async {
    try {
      ...
      debugPrint('Ошибка добавления заявки с календарем: $e');
  ...
  Future<void> updateBookingStatusWithCalendar(
    ...
    try {
      ...
      debugPrint('Ошибка обновления статуса заявки с календарем: $e');
  ...
  Future<List<DateTime>> getAvailableTimeSlots(
    ...
    Duration slotDuration = const Duration(hours: 1),
      ...
      _calendarService.getAvailableTimeSlots(specialistId, date, slotDuration);
  ...
  Future<void> _sendBookingStatusNotifications(
    ...
    try {
      ...
      if (status == 'cancelled') {
        await _notificationService.sendNotification(
            booking.specialistId ?? '', title, body,);
      ...
      debugPrint('Ошибка отправки уведомлений о заявке: $e');
    }
  }
  ...
  Future<void> _sendPaymentStatusNotifications(
    ...
    try {
      ...
      switch (status) {
        case 'completed':
          notificationType = NotificationType.paymentCompleted;
        case 'failed':
          notificationType = NotificationType.paymentCompleted;
        default:
          return; // Не отправляем уведомления для других статусов
      ...
      debugPrint('Ошибка отправки уведомлений о платеже: $e');
    }
  }
  ...
  Future<bool> markPaymentAsPaid({
    ...
    try {
      ...
      debugPrint('Ошибка обновления статуса платежа: $e');
      return false;
    }
  }
  ...
  Future<bool> isPrepaymentPaid(String bookingId) async {
    try {
      ...
      debugPrint('Ошибка проверки предоплаты: $e');
      return false;
    }
  }
  ...
  Future<PaymentStats> getUserPaymentStats(String userId) async {
    try {
      ...
      debugPrint('Ошибка получения статистики платежей: $e');
  ...
  Future<PaymentStats> getSpecialistPaymentStats(String specialistId) async {
    try {
      ...
      debugPrint('Ошибка получения статистики платежей специалиста: $e');
      return PaymentStats.empty();
    }
  }
}
...
class PaginatedResult<T> {
  const PaginatedResult(
      {required this.items, required this.hasMore, this.lastDocument,});
  final List<T> items;
