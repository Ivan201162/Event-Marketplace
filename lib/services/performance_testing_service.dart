...
class PerformanceTestingService {
  ...
  Future<Map<String, dynamic>> testFeedPerformance({
    ...
    try {
      ...
      final usersPromises = userIds.map((uid) => 
  ...
  Future<Map<String, dynamic>> testChatsPerformance({
    ...
    try {
      ...
      final messagePromises = chatsSnapshot.docs.map((chatDoc) =>
          chatDoc.reference
              .collection('messages')
              .orderBy('createdAt', descending: true)
              .limit(1)
  ...
  Future<Map<String, dynamic>> testSearchPerformance({
    ...
    try {
      ...
      final detailsPromises = specialistsSnapshot.docs.map((doc) =>
          _firestore.collection('specialist_details').doc(doc.id).get());
  ...
  Future<Map<String, dynamic>> runFullPerformanceTest({
    ...
    try {
      ...
      final allTestsSuccessful = results['tests'].values
