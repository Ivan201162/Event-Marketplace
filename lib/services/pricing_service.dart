...
class PricingService {
  ...
  Future<double> getAveragePriceByCategory(SpecialistCategory category) async {
    try {
      ...
      AppLogger.logE('Ошибка получения средней цены по категории',
          'pricing_service', e, stackTrace);
      return 0.0;
    }
  }
  ...
  Future<double> getRecommendedPrice(
      SpecialistCategory category, ExperienceLevel experienceLevel) async {
    try {
      AppLogger.logI(
  ...
  Future<void> updateAllSpecialistsAveragePrices() async {
    try {
      ...
      AppLogger.logE('Ошибка обновления средних цен всех специалистов',
          'pricing_service', e, stackTrace);
    }
  }
