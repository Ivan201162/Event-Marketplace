...
class PricingService {
  ...
  Future<Map<String, double>> calculateAveragePricesByService(String specialistId) async {
    try {
      ...
      final Map<String, List<double>> pricesByCategory = {};
  ...
  Future<double> getAveragePriceByCategory(SpecialistCategory category) async {
    try {
      ...
      AppLogger.logI('Средняя цена по категории ${category.name}: $averagePrice', 'pricing_service');
      return averagePrice;
    } catch (e, stackTrace) {
      AppLogger.logE('Ошибка получения средней цены по категории', 'pricing_service', e, stackTrace);
      return 0.0;
    }
  }
  ...
  Future<Map<String, double>> getPriceRangeByCategory(SpecialistCategory category) async {
    try {
      ...
      AppLogger.logI('Диапазон цен по категории ${category.name}: min=$minPrice, max=$maxPrice, avg=$avgPrice', 'pricing_service');
      return {
        'min': minPrice,
        'max': maxPrice,
        'avg': avgPrice,
  ...
  Future<double> getRecommendedPrice(SpecialistCategory category, ExperienceLevel experienceLevel) async {
    try {
      AppLogger.logI('Получение рекомендуемой цены для ${category.name} с уровнем ${experienceLevel.name}', 'pricing_service');
  ...
  Future<void> updateAllSpecialistsAveragePrices() async {
    try {
      ...
      AppLogger.logE('Ошибка обновления средних цен всех специалистов', 'pricing_service', e, stackTrace);
    }
  }
