...
class RecommendationService {
  ...
  Future<List<Specialist>> getCrossSellRecommendations({
    ...
    try {
      AppLogger.logI(
          'Получение cross-sell рекомендаций для ${selectedSpecialistIds.length} специалистов',
      ...
      final recommendedCategories =
          _getRecommendedCategories(selectedCategories);
      ...
      AppLogger.logI(
          'Найдено ${uniqueRecommendations.length} cross-sell рекомендаций',
          'recommendation_service');
      return uniqueRecommendations.take(6).toList(); // Максимум 6 рекомендаций
    } catch (e, stackTrace) {
      AppLogger.logE('Ошибка получения cross-sell рекомендаций',
          'recommendation_service', e, stackTrace);
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>> getBudgetIncreaseRecommendations({
    ...
    try {
      AppLogger.logI('Получение рекомендаций по увеличению бюджета',
          'recommendation_service');
      ...
      return {
        ...
        'budgetIncreasePercent':
            currentBudget > 0 ? (totalAdditionalCost / currentBudget) * 100 : 0,
      };
    } catch (e, stackTrace) {
      AppLogger.logE('Ошибка получения рекомендаций по бюджету',
          'recommendation_service', e, stackTrace);
      return {};
    }
  }
  ...
  Future<List<EventIdea>> getEventIdeas({
    ...
    try {
      ...
      AppLogger.logI(
          'Найдено ${ideas.length} идей мероприятий', 'recommendation_service');
      return ideas;
    } catch (e, stackTrace) {
      AppLogger.logE('Ошибка получения идей мероприятий',
          'recommendation_service', e, stackTrace);
      return [];
    }
  }
  ...
  Future<List<EventIdea>> getSavedIdeas(String userId) async {
    try {
      AppLogger.logI('Получение сохраненных идей пользователя: $userId',
          'recommendation_service');
      ...
      for (final ideaId in ideaIds) {
        final ideaDoc =
            await _firestore.collection('event_ideas').doc(ideaId).get();
        if (ideaDoc.exists) {
      ...
      AppLogger.logI(
          'Найдено ${ideas.length} сохраненных идей', 'recommendation_service');
      return ideas;
    } catch (e, stackTrace) {
      AppLogger.logE('Ошибка получения сохраненных идей',
          'recommendation_service', e, stackTrace);
      return [];
    }
  }
  ...
  Future<void> saveIdea(String userId, String ideaId, {String? notes}) async {
    try {
      AppLogger.logI('Сохранение идеи $ideaId пользователем $userId',
          'recommendation_service');
      ...
      AppLogger.logE(
          'Ошибка сохранения идеи', 'recommendation_service', e, stackTrace);
    }
  }
  ...
  Future<void> unsaveIdea(String userId, String ideaId) async {
    try {
      AppLogger.logI('Удаление идеи $ideaId из избранного пользователя $userId',
          'recommendation_service');
      ...
      AppLogger.logE('Ошибка удаления идеи из избранного',
          'recommendation_service', e, stackTrace);
    }
  }

  /// Получить рекомендуемые категории на основе выбранных
  Set<SpecialistCategory> _getRecommendedCategories(
      Set<SpecialistCategory> selectedCategories) {
    ...
    if (selectedCategories.contains(SpecialistCategory.caterer)) {
      recommended
          .addAll([SpecialistCategory.caterer, SpecialistCategory.decorator]);
    }
  ...
  Set<SpecialistCategory> _getMissingCategories(
      Set<SpecialistCategory> selectedCategories) {
  ...
  Future<double> _getAveragePriceForCategory(
