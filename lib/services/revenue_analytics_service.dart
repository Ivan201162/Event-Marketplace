...
class RevenueAnalyticsService {
  ...
  Future<void> recordRevenue({
    ...
    try {
      ...
      debugPrint('INFO: [RevenueAnalyticsService] Revenue recorded: $amount $currency from $sourceType');
    } catch (e) {
  ...
  Future<void> _updateAggregatedStats(RevenueStats stats) async {
    try {
      ...
      await _firestore.collection('revenue_aggregates').doc('source_${stats.sourceType.toString().split('.').last}').set({
  ...
  Future<RevenueDashboard> getRevenueDashboard({
    ...
    try {
      ...
      final DateTime prevPeriodStart = _getPeriodStart(periodStart.subtract(const Duration(days: 1)), period);
      ...
      final double averageOrderValue = totalTransactions > 0 ? totalRevenue / totalTransactions : 0.0;

      // Получаем дневную статистику
      final List<Map<String, dynamic>> dailyRevenue = await _getDailyRevenue(periodStart, periodEnd);

      // Получаем месячную статистику
      final List<Map<String, dynamic>> monthlyRevenue = await _getMonthlyRevenue(periodStart, periodEnd);
  ...
  Future<List<Map<String, dynamic>>> _getMonthlyRevenue(DateTime startDate, DateTime endDate) async {
    try {
      final List<Map<String, dynamic>> monthlyRevenue = [];
      final DateTime current = DateTime(startDate.year, startDate.month, 1);
  ...
  Future<double> _calculateLTV() async {
    try {
      ...
      final QuerySnapshot usersSnapshot = await _firestore
          .collection('user_lifetime_values')
          .get();
  ...
  Future<void> updateUserLTV(String userId) async {
    try {
      ...
      final double purchaseFrequency = daysSinceFirst > 0 ? (totalTransactions / daysSinceFirst) * 30 : 0.0;
