...
class SpecialistTipsService {
  ...
  Future<List<SpecialistTip>> getSpecialistTips(String userId) async {
    try {
      ...
      return querySnapshot.docs
          .map(SpecialistTip.fromFirestore)
          .toList();
  ...
  Future<void> _analyzeProfile(Specialist specialist, List<SpecialistTip> tips, DateTime now) async {
    // Проверка имени
    if (specialist.name.isEmpty || specialist.name.length < 2) {
      tips.add(SpecialistTip(
    ...
    if (specialist.portfolioImages.isEmpty) {
      ...
      tips.add(SpecialistTip(
        ...
        message: 'Добавьте еще ${3 - specialist.portfolioImages.length} фото для лучшего представления работ',
    ...
    if (specialist.rating < 4.0 && specialist.reviewCount > 0) {
      tips.add(SpecialistTip(
        ...
        message: 'Ваш рейтинг ${specialist.rating.toStringAsFixed(1)}. Работайте над улучшением качества услуг',
  ...
  Future<int> cleanupOldCompletedTips() async {
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        await batch.commit();
        print('Удалено ${querySnapshot.docs.length} старых выполненных советов');
      }
