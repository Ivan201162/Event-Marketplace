...
class MonitoringService {
  ...
  void endOperation(String operationName, {Map<String, dynamic>? metadata}) {
    ...
    if (startTime == null) {
      _logger.warning('Operation $operationName was not started', tag: 'MONITORING');
      return;
    }
    
    final duration = DateTime.now().difference(startTime);
    _operationTotalTimes[operationName] = (_operationTotalTimes[operationName] ?? Duration.zero) + duration;
    ...
    _logger.performance(operationName, duration, tag: 'MONITORING', data: metadata);
  ...
  void _recordError(String errorType, Object error, StackTrace? stack) {
    ...
    _logger.error('Error recorded: $errorType', tag: 'MONITORING', error: error, stackTrace: stack);
  }
  ...
  void _collectMemoryMetrics() {
    try {
      ...
      _logger.debug('Memory usage: ${memoryUsage ~/ 1024 / 1024}MB', tag: 'MONITORING');
      
      // Если использование памяти превышает 200MB, логируем предупреждение
      if (memoryUsage > 200 * 1024 * 1024) {
        _logger.warning('High memory usage detected: ${memoryUsage ~/ 1024 / 1024}MB', tag: 'MONITORING');
      }
    } catch (e) {
      _logger.error('Failed to collect memory metrics', tag: 'MONITORING', error: e);
    }
  }
  ...
  void _cleanupOldMetrics() {
    ...
    _performanceMetrics.removeWhere((metric) => metric.timestamp.isBefore(cutoffTime));
    _errorMetrics.removeWhere((metric) => metric.timestamp.isBefore(cutoffTime));
  ...
  Map<String, dynamic> getErrorStats() {
    ...
    stats['recentErrors'] = _errorMetrics
        .where((e) => e.timestamp.isAfter(DateTime.now().subtract(Duration(minutes: 5))))
        .length;
  ...
  Map<String, dynamic> getMemoryStats() {
    ...
    final maxMemory = _memoryMetrics.map((m) => m.memoryUsage).reduce((a, b) => a > b ? a : b);
    final minMemory = _memoryMetrics.map((m) => m.memoryUsage).reduce((a, b) => a < b ? a : b);
    final avgMemory = _memoryMetrics.map((m) => m.memoryUsage).reduce((a, b) => a + b) / _memoryMetrics.length;
