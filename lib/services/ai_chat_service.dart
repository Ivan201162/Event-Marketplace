...
class AiChatService {
  ...
  Future<Map<String, dynamic>> _processUserMessage(String message, UserContext context) async {
    ...
    if (_isGreeting(lowerMessage)) {
      return {
        'content': 'Привет! Я AI-помощник Event Marketplace. Помогу подобрать специалистов для вашего мероприятия. Расскажите, какое событие планируете?',
    ...
    if (_isEventType(lowerMessage)) {
      ...
      return {
        'content': 'Отлично! ${_getEventTypeDescription(eventType)} В каком городе планируется мероприятие?',
    ...
    return {
      'content': 'Извините, не совсем понял. Можете уточнить? Я помогу подобрать специалистов для вашего мероприятия.',
      'type': MessageType.question.value,
      'metadata': {
        'quickReplies': [
    ...
    try {
      ...
      final filteredSpecialists = specialists.where((specialist) {
        ...
        if (context.eventType != null) {
          ...
          if (eventType == 'wedding' && !['photographer', 'videographer', 'host', 'dj'].contains(category)) {
            matches = false;
          } else if (eventType == 'corporate' && !['host', 'dj', 'caterer'].contains(category)) {
            matches = false;
          } else if (eventType == 'birthday' && !['photographer', 'host', 'dj'].contains(category)) {
            matches = false;
          }
        }
      ...
      if (topSpecialists.isEmpty) {
        return {
          'content': 'К сожалению, не нашел подходящих специалистов по вашим критериям. Попробуйте изменить параметры поиска.',
          'type': MessageType.text.value,
          'metadata': {
            'quickReplies': [
  ...
  bool _isGreeting(String message) {
    final greetings = ['привет', 'здравствуйте', 'добрый день', 'добрый вечер', 'доброе утро', 'hi', 'hello'];
    return greetings.any((greeting) => message.contains(greeting));
  }

  bool _isEventType(String message) {
    final eventTypes = ['свадьба', 'корпоратив', 'день рождения', 'юбилей', 'праздник', 'мероприятие'];
  ...
  String _extractEventType(String message) {
    ...
    if (message.contains('день рождения') || message.contains('др')) return 'birthday';
  ...
  String _getEventTypeDescription(String eventType) {
    switch (eventType) {
      case 'wedding': return 'Свадьба - это особенный день!';
      case 'corporate': return 'Корпоративное мероприятие - отличный выбор!';
      case 'birthday': return 'День рождения - замечательный повод для праздника!';
      case 'anniversary': return 'Юбилей - важное событие!';
      default: return 'Интересное мероприятие!';
    }
  }
  ...
  String _extractCity(String message) {
    ...
    if (message.contains('санкт-петербург') || message.contains('спб')) return 'Санкт-Петербург';
    if (message.contains('екатеринбург')) return 'Екатеринбург';
    if (message.contains('новосибирск')) return 'Новосибирск';
    return 'Другой город';
  }

  bool _isBudget(String message) => message.contains('бюджет') || message.contains('₽') || message.contains('руб') || 
