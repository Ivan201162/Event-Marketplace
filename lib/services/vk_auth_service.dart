...
class VKAuthService {
  ...
  Future<void> startVkLoginWeb() async {
    try {
      ...
      AppLogger.logE('Ошибка начала VK OAuth', 'vk_auth_service', e, stackTrace);
  ...
  Future<AppUser?> handleVkCallback(String code) async {
    try {
      ...
      AppLogger.logE('Ошибка обработки VK callback', 'vk_auth_service', e, stackTrace);
  ...
  Future<String?> _exchangeCodeForCustomToken(String code) async {
    try {
      AppLogger.logI('Обмен кода на custom token через Cloud Function', 'vk_auth_service');
      
      // Вызываем Cloud Function для получения custom token
      final callable = FirebaseFunctions.instance.httpsCallable('vkCustomToken');
      ...
      AppLogger.logE('Ошибка обмена кода на token', 'vk_auth_service', e, stackTrace);
      return null;
    }
  }
  ...
  Future<AppUser?> createVkUserForTesting() async {
    try {
      ...
      final firebaseUser = credential.user;
      if (firebaseUser == null) {
        AppLogger.logE('Firebase пользователь не создан для VK', 'vk_auth_service');
        return null;
      }
      ...
      AppLogger.logI('VK пользователь создан: ${appUser.displayName}', 'vk_auth_service');
      return appUser;
      
    } catch (e, stackTrace) {
      AppLogger.logE('Ошибка создания VK пользователя', 'vk_auth_service', e, stackTrace);
