...
class _HomeSearchFiltersWidgetState extends ConsumerState<HomeSearchFiltersWidget> {
    ...
    children: [
      ...
      DropdownButtonFormField<String>(
        ...
        items: [
          ...
          ..._categories.map(
            (category) =>
                DropdownMenuItem<String>(value: category['value'], child: Text(category['name'])),
          ),
        ],
        onChanged: (value) {
          setState(() {
  ...
  Widget _buildDateFilter() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('Дата мероприятия', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
      const SizedBox(height: 8),
      InkWell(
        onTap: _selectDate,
        child: Container(
