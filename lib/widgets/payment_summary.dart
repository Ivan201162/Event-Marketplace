...
class PaymentSummary extends StatelessWidget {
  const PaymentSummary({
    required this.amount, required this.type, required this.taxAmount, required this.netAmount, super.key,
  });
  final double amount;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Amount breakdown
          _buildAmountRow(
              theme, 'Сумма к оплате', '${amount.toStringAsFixed(0)} ₽',
              isTotal: true,),

          if (taxAmount > 0) ...[
            const SizedBox(height: 12),
            _buildAmountRow(
                theme, 'Налог (13%)', '${taxAmount.toStringAsFixed(0)} ₽',),
          ],

          if (netAmount > 0) ...[
            const SizedBox(height: 12),
            _buildAmountRow(
                theme, 'К получению', '${netAmount.toStringAsFixed(0)} ₽',),
          ],
