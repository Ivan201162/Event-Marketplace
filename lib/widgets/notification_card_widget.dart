...
class _NotificationCardWidgetState extends ConsumerState<NotificationCardWidget>
  ...
  Widget _buildNotificationIcon() => Container(
    ...
    child: Center(child: Text(widget.notification.type.icon, style: const TextStyle(fontSize: 20))),
  );
  ...
  Widget _buildNotificationDetails() => Column(
    ...
    children: [
      ...
      _buildDetailRow('Приоритет', widget.notification.priority.displayName, Icons.priority_high),
      if (widget.notification.expiresAt != null)
        _buildDetailRow('Истекает', _formatDate(widget.notification.expiresAt!), Icons.schedule),
      if (widget.notification.readAt != null)
        _buildDetailRow('Прочитано', _formatDate(widget.notification.readAt!), Icons.check_circle),
    ],
  );
    ...
    children: [
      ...
      PopupMenuButton<String>(
        ...
        itemBuilder: (context) => [
          ...
          const PopupMenuItem(
            value: 'archive',
            child: Row(children: [Icon(Icons.archive), SizedBox(width: 8), Text('Архивировать')]),
          ),
          const PopupMenuItem(
            value: 'delete',
            child: Row(
