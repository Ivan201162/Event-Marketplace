...
class BadgeWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedButton(
    ...
    child: Container(
      ...
      child: Center(
        child: Text(badge.icon, style: TextStyle(fontSize: size * 0.4)),
      ),
    ),
  );
...
class BadgeInfoWidget extends StatelessWidget {
  const BadgeInfoWidget({super.key, required this.badge, this.isCompact = false});
  final badge_model.Badge badge;
  ...
  Widget _buildCompactView(BuildContext context) => AnimatedCard(
    child: Row(
      children: [
        BadgeWidget(badge: badge, size: 40),
  ...
  Widget _buildFullView(BuildContext context) => AnimatedCard(
    child: Column(
      children: [
        BadgeWidget(badge: badge, size: 80),
...
class BadgeCollectionWidget extends ConsumerWidget {
      ...
      data: (badges) {
        ...
        return Column(
          ...
          children: [
            ...
            if (category == null)
              _buildGridLayout(context, filteredBadges.cast<badge_model.Badge>())
            else
              _buildListLayout(context, filteredBadges.cast<badge_model.Badge>()),
          ],
        );
      },
  ...
  Widget _buildGridLayout(BuildContext context, List<badge_model.Badge> badges) => Wrap(
    spacing: 12,
    runSpacing: 12,
    children: badges
        .map(
          (badge) =>
              BadgeWidget(badge: badge, size: 50, onTap: () => _showBadgeDetails(context, badge)),
        )
        .toList()
        .cast<Widget>(),
  );

  Widget _buildListLayout(BuildContext context, List<badge_model.Badge> badges) => Column(
    children: badges
        .map(
          (badge) => Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: BadgeInfoWidget(badge: badge, isCompact: true),
          ),
        )
        .toList()
        .cast<Widget>(),
...
class BadgeStatsWidget extends ConsumerWidget {
  ...
  Widget _buildStatItem(BuildContext context, String label, int count, IconData icon) => Column(
    children: [
...
class BadgeLeaderboardWidget extends ConsumerWidget {
  const BadgeLeaderboardWidget({super.key, this.limit = 10});
  final int limit;
