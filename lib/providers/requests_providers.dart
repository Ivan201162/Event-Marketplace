...
final requestsByStatusProvider = FutureProvider.family<List<Request>, ({String userId, RequestStatus status})>((ref, params) async {
  final service = ref.read(requestServiceProvider);
  return await service.getRequestsByStatus(params.userId, params.status);
});

/// Requests by category provider
final requestsByCategoryProvider = FutureProvider.family<List<Request>, String>((ref, category) async {
  final service = ref.read(requestServiceProvider);
